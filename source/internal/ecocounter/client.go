...
const (
	...
	DefaultBaseURL = "https://www.eco-visio.net"
)
...
func (c Client) GetDatapoints(id string, begin, end time.Time, resolution Resolution) ([]Datapoint, error) {
	...
	u.Path = path.Join(u.Path, "/api/aladdin/1.0.0/pbl/publicwebpage/data/"+meta.flowid)
	...
	if resp.StatusCode != http.StatusOK {
		...
		return nil, fmt.Errorf("GetDatapoints: bad status for id %q: %d, %q, %q", id, resp.StatusCode, req.URL, string(b))
	}
...
func (c Client) getPublicMeta(id string) (publicMeta, error) {
	u, err := c.baseURL()
	if err != nil {
		return publicMeta{}, err
	}

	u.Path = path.Join(u.Path, "/api/aladdin/1.0.0/pbl/publicwebpage/"+id)

	req, err := http.NewRequest(http.MethodGet, u.String(), nil)
	if err != nil {
		return publicMeta{}, fmt.Errorf("getPublicMeta: error making request for id %q: %s", id, err)
	}

	resp, err := c.do(req)
	if err != nil {
		return publicMeta{}, fmt.Errorf("getPublicMeta: error requesting data for id %q: %s", id, err)
	}
	defer resp.Body.Close()

	if resp.StatusCode != http.StatusOK {
		return publicMeta{}, fmt.Errorf("getPublicMeta: bad status for id %q: %d", id, resp.StatusCode)
	}
	...
	if err := json.NewDecoder(resp.Body).Decode(&body); err != nil {
		return publicMeta{}, fmt.Errorf("getPublicMeta: decoding response body for id %q: %s", id, err)
	}

	pm := publicMeta{
		domain: strconv.Itoa(body.Domaine),
		token:  body.Token,
		flowid: strconv.Itoa(body.FlowID),
	}
	return pm, nil
}
